...
def get_params(results, exercise="squats", all=False):
    ...
    theta_s1 = get_angle(
        points["LEFT_ELBOW"] - points["LEFT_SHOULDER"],
    ...
    theta_h1 = get_angle(
        points["RIGHT_KNEE"] - points["RIGHT_HIP"],
    ...
    z1 = (points["RIGHT_ANKLE"][2] + points["RIGHT_HEEL"][2]) / 2 - points[
        "RIGHT_FOOT_INDEX"
    ][2]

    z2 = (points["LEFT_ANKLE"][2] + points["LEFT_HEEL"][2]) / 2 - points[
        "LEFT_FOOT_INDEX"
    ][2]
    ...
    left_foot_y = (
        points["LEFT_ANKLE"][1] + points["LEFT_HEEL"][1] + points["LEFT_FOOT_INDEX"][1]
    ) / 3
    right_foot_y = (
        points["RIGHT_ANKLE"][1]
        + points["RIGHT_HEEL"][1]
        + points["RIGHT_FOOT_INDEX"][1]
    ) / 3
    ...
    theta_left_foot = get_angle(
        left_foot, np.array([left_foot[0], left_foot[1], points["LEFT_FOOT_INDEX"][2]])
    )
    right_foot = points["RIGHT_HEEL"] - points["RIGHT_FOOT_INDEX"]
    theta_right_foot = get_angle(
        right_foot,
        np.array([right_foot[0], right_foot[1], points["RIGHT_FOOT_INDEX"][2]]),
    )
    ...
    if all:
        params = (
            np.array([[x, y, z] for pos, (x, y, z) in points.items()])
